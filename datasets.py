...
class Wallach(data.Dataset):
    def __init__(self, args, is_cropped=False, root='', dstype='images', replicates=1):
        self.args = args
        self.is_cropped = is_cropped
        self.crop_size = args.crop_size
        self.render_size = args.inference_size
    ...
    def __getitem__(self, index):
        ...
        index = index % self.size
        #print("Get item called: ",index, " Size:",self.size)
        img1 = frame_utils.read_gen(self.image_list[index][0])
        img2 = frame_utils.read_gen(self.image_list[index][1])
...
class FlyingChairs(data.Dataset):
    ...
    def __getitem__(self, index):
        index = index % self.size
        print("Get item: ", index, ":", self.size)
        img1 = frame_utils.read_gen(self.image_list[index][0])
        img2 = frame_utils.read_gen(self.image_list[index][1])
