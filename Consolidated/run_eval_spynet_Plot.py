...
if __name__ == '__main__':
	...
	flow_dir =   '/home/4TBInt/OpticalFlow/Data/ColorVersion_CC1' 
        out_dir = '/home/4TBInt/OpticalFlow/Results/Spynet_ColorVersion_CC1'
        out_dir_summary = '/home/4TBInt/OpticalFlow/Results/Spynet_ColorVersion_CC1_Summary'
	#flow_dir = '/home/SDD_Work/ColorVersion_CC1'
        #out_dir = '/home/SDD_Work/Results/Spynet_ColorCC1'
        #out_dir_summary = '/home/SDD_Work/Results/Spynet_ColorCC1_Summary'
        
        
        if not os.path.exists(out_dir):
            os.mkdir(out_dir)
        if not os.path.exists(out_dir_summary):
            os.mkdir(out_dir_summary)
	img_list, flow_list = getFileList(flow_dir)
	epe_all = np.zeros((len(img_list)))
		...
		flow = Flow()
		fig_file_name = join(out_dir_summary,flow_list[i][len(flow_dir)+1:-4]+'_Spynet_SintenClean_predicted.png')        
                ...
                #print("No of bad pixels:")
                #print(np.sum(np.isnan(pred_flow_vis.flatten())==1))
                fig=plt.figure()
                img1 = numpy.array(PIL.Image.open(img_list[i][0]))
                img2 = numpy.array(PIL.Image.open(img_list[i][1]))
		...
		#f, (ax1, ax2, ax3,ax4) = plt.subplots(2,2,figsize=(10,3),sharey=True)
		f, ax = plt.subplots(2,2,sharex=True,sharey=True)
		ax1 = ax[0,0]
		ax2 = ax[0,1]
		ax3 = ax[1,0]
		ax4 = ax[1,1]
                ax1.imshow(img1),ax1.set_title('First frame')
                ax2.imshow(img2),ax2.set_title('Second frame')
                ax3.imshow(gt_flow_vis),ax3.set_title('Ground truth')
                ax4.imshow(pred_flow_vis),ax4.set_title('EPE: '+str(epe_flow))
		...
		ax3.tick_params(top='off', bottom='off', left='off', right='off', labelleft='off', labelbottom='off')
		ax4.tick_params(top='off', bottom='off', left='off', right='off', labelleft='off', labelbottom='off')

		f.tight_layout()
                f.savefig(fig_file_name, bbox_inches='tight', pad_inches = 0, format='png', dpi=300)
		#plt.show()        
		plt.close()
		outfile_name = join(out_dir,flow_list[i][len(flow_dir)+1:-4]+'_Spynet_predicted.flo')        
		...
		objectOutput.close()
		#print("___")
	np.save('Spynet_PMD_EPE_CC1_All.npy', epe_all)
# end
